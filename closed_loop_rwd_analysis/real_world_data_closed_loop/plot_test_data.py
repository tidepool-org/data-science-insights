"""
Plot the raw test data from dev.closed_loop_rwd.test_closed_loop_device_data.

4-panel time-series: CBG, rec type, bolus events, carb announcements.
8 day-range groups shaded and labeled.
"""
...
ax = axes[2]
if len(boluses) > 0:
    # Color by meal vs correction
    meal = boluses[boluses["food"].notna()]
    correction = boluses[boluses["food"].isna()]
    if len(correction) > 0:
        ax.stem(correction["ts"], correction["bolus_units"],
                linefmt="C0-", markerfmt="C0o", basefmt="gray", label="Correction")
    if len(meal) > 0:
        ax.stem(meal["ts"], meal["bolus_units"],
                linefmt="C3-", markerfmt="C3o", basefmt="gray", label="Meal")
    ax.legend(loc="upper right", fontsize=8)
...
# 8 groups × 5 days each, starting 2024-01-01
day_ranges = [
    ("2024-01-01", "2024-01-06", "TB NoBolus\nNoFood",   "#4A90D9"),
    ("2024-01-06", "2024-01-11", "AB NoBolus\nNoFood",   "#D94A4A"),
    ("2024-01-11", "2024-01-16", "TB NoBolus\n+Food",    "#4A90D9"),
    ("2024-01-16", "2024-01-21", "AB NoBolus\n+Food",    "#D94A4A"),
    ("2024-01-21", "2024-01-26", "TB FCL\nCorrection",   "#4A90D9"),
    ("2024-01-26", "2024-01-31", "AB FCL\nCorrection",   "#D94A4A"),
    ("2024-01-31", "2024-02-05", "TB HCL",               "#4A90D9"),
    ("2024-02-05", "2024-02-10", "AB HCL",               "#D94A4A"),
]
for i, (start, end, label, color) in enumerate(day_ranges):
    for ax in axes:
        ax.axvspan(start, end, alpha=0.07, color=color)
    # Stagger labels across 4 rows to avoid overlap
    y_frac = [0.97, 0.87, 0.77, 0.67][i % 4]
    axes[0].text(
        mdates.datestr2num(start), axes[0].get_ylim()[1] * y_frac,
        label, fontsize=7, color=color, fontweight="bold", va="top",
    )

fig.suptitle(f"Raw Test Data: {TABLE}", fontsize=14, fontweight="bold")
fig.tight_layout()
plt.savefig("plot_test_closed_loop_data.png", dpi=150, bbox_inches="tight")
plt.show()
