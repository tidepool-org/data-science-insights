"""
FCL vs HCL: Autobolus Users Only

For Loop autobolus users (v<3.4) with >=10 FCL days, compare glycemic
endpoints between FCL and HCL days.

Day classification:
  - FCL: no food announcements AND ≤1 correction bolus
  - HCL: everything else (food, meal boluses, or >1 correction)

Qualifying: >=10 FCL days. Filtered to autobolus days only.
"""
...
# Filter to autobolus days only, FCL and HCL
pdf = pdf[
...
GROUPS = ["fcl", "hcl"]
GROUP_LABELS = ["FCL", "HCL"]

groups = {g: pdf[pdf["loop_mode"] == g] for g in GROUPS}

fcl_users = pdf[pdf["loop_mode"] == "fcl"]["_userId"].nunique()
print(f"Users with autobolus FCL days:  {fcl_users:,}")
print(f"Autobolus days (FCL + HCL):        n={len(pdf):,}")
print()
...
# =============================================================================
# Plot: Stacked bar — FCL vs HCL (autobolus only)
# =============================================================================
...
ax.set_title("FCL vs HCL (Autobolus Users)",
             fontsize=14, fontweight="bold")
ax.grid(axis="y", alpha=0.3)
plt.tight_layout()
plt.savefig("fcl_optimized_stacked_bar.png", dpi=150, bbox_inches="tight")
plt.show()

# =============================================================================
# Plot: Box plots — FCL vs HCL (autobolus only)
# =============================================================================
...
fig.suptitle("FCL vs HCL (Autobolus Users)",
             fontsize=14, fontweight="bold")
fig.tight_layout()
plt.savefig("fcl_optimized_boxplots.png", dpi=150, bbox_inches="tight")
plt.show()
