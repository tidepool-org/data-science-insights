...
Writes to dev.fda_510k_rwd.test_no_bolus_device_data with the same column
format as bddp_sample_all. One user, 20 days:

  - Days  0–4:   TB + No Bolus  (temp_basal recs, no dosingDecision)  5 days
  - Days  5–9:   AB + No Bolus  (autobolus recs, no dosingDecision)   5 days
  - Days 10–14:  TB + HCL       (temp_basal, meal bolus + food)       5 days
  - Days 15–19:  AB + HCL       (autobolus, meal bolus + food)        5 days

The user has 10 no-bolus days, so qualifies (>=10 no-bolus/FCL days).

Each day has 200 CBG readings at 5-min intervals with fixed mg/dL values
per glycemic bucket, 10 Loop recommendation rows, and (for HCL days)
a dosing decision + food announcement row.
"""
