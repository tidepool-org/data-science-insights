...
- **RFAI — insulin type by cohort (2026-09-09; Phase 0 + 0b run, FDA letter read, N delivered)**: plan in Drive `510k/claude/RFAI/PLN_RFAI_insulin_type_draft_2026-09-09.md` (§0 status, §1 FDA verbatim, §11.3 the N tables, §12 Part B proposal); FDA letter K262435 + text extract beside it. The ask is Clinical deficiency 2 (p. 3): the Loop Observational Study captured insulin type only at baseline, so Fiasp outcomes are not attributable to confirmed, sustained use, and Lyumjev had no users — FDA wants clinical or credible simulation data for Fiasp/Lyumjev under the algorithm, or their removal from labeling (D1 closed: brand level, URAI Fiasp/Lyumjev vs RAI Novolog/Humalog). Data sources settled: per-dose insulin type reaches BDDP by two upload paths — the Tidepool-plugin path as `insulinFormulation.simple.brand` (records with `origin.version`) and the HealthKit path as `payload["com.loopkit.InsulinKit.MetadataKeyInsulinType"]` (string; same metadata the HealthKit autobolus classification reads); the two agree on >99% of shared days; `pumpSettings.insulinModel` carries no brand information and is dropped (D4 closed). Because autobolus and insulin-type selection are both Loop 3 features, every eligible transition window and essentially every eligible AB day carries a confirmed type — no carry/fallback needed; confirmed same-day type is the only source used. Probes: `exploratory/rfai_insulin_type_probe.py` (Phase 0) and `exploratory/rfai_insulin_type_probe_b.py` (0b; section 10 prints the N tables per cohort; writes scratch `rfai_tmp_brand_by_day`, drop when Phase 1 replaces it). `exploratory/rfai_insulin_type_by_age.sql` (self-contained, no scratch table): brand × pediatric band (6–11 / 12–17 / 18+) for the 0.80-box §8-1 transition cohort (users, age at seg1 start) and for all eligible AB days (days + users, age on the day), brands read directly from the raw BDDP dose rows; also the per-user transition listing. Tie-outs: IR-2 outcome users and the five guardrail groups match the IR-1002 production counts exactly; the §8-1 analysis-cohort N from the SQL predicate still needs checking against Table 6.3a. Counts live in the Drive plan doc only. Next: decide D9 (Part B — outcomes on AB days by confirmed insulin class, day-grain attribution with sustained-use runs; within-user switchers strongest) and D3 (how to present users with more than one brand); Phase 1 staging (`export_insulin_type_by_day.py` = production form of the scratch table, plus a runs table) only once the response scope is settled.
