...
**IR-6B — preset dose-response plots (2026-08-24 → 25)**: FDA follow-up asking for six plots — TB70/TIR/TAR vs overall insulin % (15–200%) and vs target range (67–200 mg/dL). Built and run: `exploratory/ir-6b_episode_counts.sql` (Phase 0 scoping, run clean), `analysis/analysis_ir-6b_dose_response.py` (production run green — event-anchored episodes on two grains: C1 = transition activations split TB/AB by rank-1 segment, C2 = per-activation guardrail-compliant AB-day activations from the staged IR-1002 flags; filter 4 = full window disjointness; hard invariants halt the run on violation; intermediates + `--figures-only` for Spark-free figure iteration; ADA reference lines behind `--no-ada-lines`; during/target-floor figures set aside behind `GENERATE_SET_ASIDE_FIGURES`), `testing/integration/test_analysis_ir_6b.py` + archetypes int_user_33..40 (green on Databricks; re-pinned for the tightened filter — one re-run pending). Report scaffold + plan in Drive `510k/claude/IR-1006/`. Next: re-run test + production after the filter tightening, fill the scaffold's [RUN] placeholders, response drafting. History entries 2026-08-24/25.
