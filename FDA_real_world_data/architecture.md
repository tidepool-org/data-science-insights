...
**RFAI (FDA Additional Information request) — insulin type by cohort (2026-09-09 → 09-15; Part A delivered, Part B plan next)**: FDA's Additional Information letter (Clinical deficiency 2) faults the Loop Observational Study for recording insulin type only at baseline — Fiasp outcomes not attributable to confirmed, sustained use; no Lyumjev users — and asks for clinical or credible simulation evidence for Fiasp / Lyumjev under the algorithm, or their removal from labeling. The device data carry a **per-dose insulin type**: Loop 3 stamps the pod's insulin type on every dose it uploads, as `insulinFormulation.simple.brand` on the Tidepool-plugin path (records with `origin.version`) and as `payload["com.loopkit.InsulinKit.MetadataKeyInsulinType"]` on the HealthKit path (the same metadata the HealthKit autobolus classifier reads); `pumpSettings.insulinModel` holds only Loop's default rapid-acting curve and is not a brand source. Because autobolus detection and insulin-type selection both arrived with Loop 3, every eligible dosing day and every transition window carries a confirmed type — no carry, no imputation. Three exploratory scripts (see the directory map): the Phase 0 probe (schema / coverage by upload path), the Phase 0b probe (S1 ∪ S3 decode, scratch table `rfai_tmp_brand_by_day`, N tables per cohort), and `rfai_insulin_type_by_age.sql` (self-contained; brand × age band for the 0.80-box §8-1 cohort and for eligible autobolus and temp-basal days; salted aliases only). Attribution rules: confirmed same-day type; a window is one brand, `multiple`, or `unknown`; class roll-up RAI (Novolog, Humalog) vs URAI (Fiasp, Lyumjev), Apidra off-label, Afrezza never a window brand. **Every count lives in Drive `510k/claude/RFAI/`** — plan `PLN_RFAI_insulin_type_draft_2026-09-09.md` (§0 status, §11 results, §12 Part B proposal) and the landscape deliverable `RFAI_insulin_type_tables_2026-09-15.docx` — none in the repo. Next: the Part B plan (glycemic outcomes on autobolus days by confirmed insulin class, day-grain attribution with sustained-use runs, within-user switchers), decision D9; regenerate a current-vintage Table 6.3a with the type-1 stage before quoting the transition N; drop the scratch table once Phase 1 staging replaces it.

**IR-6B — preset dose-response plots (2026-08-24 → 26)**: FDA follow-up asking for six plots — TB70/TIR/TAR vs overall insulin % (15–200%) and vs target range (67–200 mg/dL). Built, run and re-run: `exploratory/ir-6b_episode_counts.sql` (Phase 0 scoping; header notes it keeps the original seg1..seg3 C1 window), `analysis/analysis_ir-6b_dose_response.py` (production re-run green 2026-08-26 — event-anchored episodes on two grains: C1 = transition activations split temp-basal/autobolus as the SYMMETRIC fortnight pair seg1 | seg2 (seg3 retained in the machinery as `transition_phase_extended`, outside the report series), C2 = per-activation guardrail-compliant AB-day activations from the staged IR-1002 flags; filter 4 = full window disjointness; hard invariants halt the run on violation; intermediates are pseudonymized at write (salted SHA-256) so the Spark-free `--figures-only` loop can run locally; every figure draws its median line through every bucket with markers encoding the 5-user support threshold; ADA reference lines behind `--no-ada-lines`; during/target-floor figures set aside behind `GENERATE_SET_ASIDE_FIGURES`, with the during/pre companion columns computed but unreferenced in the report), `testing/integration/test_analysis_ir_6b.py` + archetypes int_user_33..40 (re-pinned for seg2-only C1b and the output-artifact cases 7b/7c; GREEN on Databricks 2026-08-26). Rendered deliverables in Drive `510k/claude/IR-1006/`: the main report (C1-first, captions below figures, funnel as one 5-column transition table + C2 table via `build_funnel_block.py`) and the supplementary-materials docx (provenance, plotted line values, verification checks), both through `postprocess_ir6b_docx.py` (Arial/black/10pt, autofit zero-padding tables, landscape final section). Next: team review, FDA response drafting. History entries 2026-08-24 → 26.

**IR6 — extreme-preset outcomes (2026-08-19 → 21)**: FDA asks for CGM outcomes and adverse events when subjects used presets at the extremes of the to-be-marketed guardrail envelope — insulin needs at 15% / 200%, and target ranges in the 67–100 / 180–250 mg/dL bands. Decisions: extreme = **at or beyond** the bound; **FDA's principal concern is the insulin-needs scale factor** (needs extremes = analytic core, decomposed by concurrent target band via the 2×2 grid; target-band marginals informational); **AEs are handled from the Jaeb study data outside this pipeline** (hypo events stay as ordinary glycemic endpoints). Scoping (three grains) + user co-occurrence complete via `exploratory/ir-6_extreme_preset_counts.sql` / `ir-6_extreme_preset_summary.sql`; grain A whole-segment outcomes complete via `analysis/analysis_ir-6_extreme_preset_outcomes.py` (run 2026-08-21, all checks pass). Plan + results docs in Drive `510k/claude/IR-1006/`. Open: grain C outcomes companion decision; response drafting. History entries 2026-08-19/20/21.

**Validity-box namespaces (2026-08-05)**: the **unsuffixed** build is the report primary (symmetric 0.80 box) — `export_valid_transition_segments.py` defaults to 0.20/0.80, so the pipeline job and any bare analysis run produce it. `_box070` (0.30/0.70) and `_box090` (0.10/0.90) are sensitivity variants that appear only when requested, via `production_runs/run_all_boxes.py` or the `Variant_Box070` / `Variant_Box090` pipeline tasks. `run_transition_variant.py` defaults to the unsuffixed primary; a drift test pins the staging defaults, the driver defaults and `BOX_CONFIGS` together. `fda_analysis_pipeline.yml` is turnkey: base tables → primary build → analyses → IR-1002 → sensitivity variants, in that order.
...
The **TB→AB validity box is now configurable end-to-end**: `export_valid_transition_segments.py` takes `autobolus_low` / `autobolus_high` `run()` params (defaults 0.20 / 0.80 since 2026-08-05 — the report primary), the analysis loaders + transition analyses (8-1/2/3/4/5/8) take a `suffix`, and `exploratory/run_transition_variant.py` rebuilds the box-affected subtree into parallel `{suffix}` tables + `outputs/analysis_8_X{suffix}/` folders to evaluate a different box without touching the primary (`_box070` / `_box090`). 8-6/8-7 (stable-AB / durability) are box-independent and untouched.
...
**Per-user diagnosis-type lookup**: `data_staging/export_user_diagnosis_type.py` builds `user_diagnosis_type` (one row per FDA Loop user — distinct `_userId` in `loop_recommendations`) carrying the diabetes diagnosis from `prod.default.patients` and `prod.default.seagull_profiles` (kept as separate `diagnosis_patients` / `diagnosis_seagull` columns), a JAEB-cohort flag, an `is_lada` contingency flag (raw labels, patients-then-seagull precedence, evaluated *before* the JAEB override; unread by the primary pipeline), and a resolved `diagnosis_type` (JAEB → `type1`, else patients, else seagull). `exploratory/cohort_diagnosis_breakdown.sql` reports the diagnosis mix across all three analysis cohorts (transition / stable / durability) by joining each cohort to that lookup; `cohort_diagnosis_type.sql` (single-cohort, direct patients join) and `preset_counts.sql` (§8-4 preset-activation counts) are companion exploratory queries.
...
│   ├── export_ab_day_cohort.py                  — IR-1002: one row per (type-1 user, day) with every day gate as a flag (is_ab_day at ≥3 boluses, version, age, coverage, is_eligible_ab_day, is_outcome_day) + per-user first_eligible_ab_day; default-off `--include_lada` widens the gate to type1 OR is_lada and carries is_lada (LADA contingency — `_lada` parallel builds only, never the primary)
...
│   └── export_user_diagnosis_type.py            — Build user_diagnosis_type: per-user diabetes diagnosis from prod patients + seagull_profiles, JAEB cohort → type1 override; FDA Loop-user universe (loop_recommendations); is_lada flag from the raw labels pre-override (contingency tracking, unread by the primary pipeline)
│
├── analysis/
...
│   ├── analysis_ir-6_extreme_preset_outcomes.py — IR6: WHOLE-SEGMENT glycemic outcomes (S1/S2/S3) for extreme-preset users — 2 needs marginals + 4 grid cells, fixed membership (any qualifying extreme activation in the window; per-bar active-vs-carried split), endpoints computed in-script from loop_cbg × rank-1 windows via compute_glycemic_endpoints() (glycemic_endpoints_transition lacks seg3; S1/S2 cross-checked against it), no coverage gate (annotations instead). Eight categories (4 marginals + 4 grid cells), every figure split marginals-vs-grid: Tables IR-6a outcomes / IR-6b checks; Figures IR-6a/6b stacked ranges, IR-6c–6f violin panels (within-user S1→S2→S3 trajectories, active/carried markers)
...
│
├── exploratory/
│   ├── rfai_insulin_type_probe.py          — RFAI Phase 0 (read-only): which insulin-type fields exist on the BDDP extract (pumpSettings.insulinModel; per-dose insulinFormulation), their distribution by Loop version / year, coverage inside the eligible transition segments and eligible AB days
│   ├── rfai_insulin_type_probe_b.py        — RFAI Phase 0b: decodes the HealthKit-path insulin type (payload MetadataKeyInsulinType) alongside the plugin-path formulation, labels each dose record's upload path, materialises scratch `rfai_tmp_brand_by_day`, checks S1-vs-S3 agreement, and prints the N tables per cohort (§8-1 transition cohort, eligible segments, IR-1002 AB-day users, IR-2 outcome users, guardrail groups, AB days day-weighted); drop the scratch table when staging replaces it
│   ├── rfai_insulin_type_by_age.sql        — RFAI deliverable query, self-contained (no scratch table): rebuilds the 0.80-box §8-1 cohort and the eligible dosing days (autobolus / temp-basal per the transition staging's rule), reads every brand straight from the raw bolus/basal rows (both fields, distinct dose times per brand, deterministic pod-change tie-break), and prints brand × age band (6–11 / 12–17 / 18+ / unknown) for transition users and dosing days plus a per-user listing keyed on a salted alias; tolerates non-ASCII digits in bddp_user_dates.dob
...
│   ├── run_transition_variant.py          — Driver: switch the validity box (`--suffix`/`--autobolus-low`/`--autobolus-high`/`--skip-analysis`), rebuild the box-affected transition subtree into parallel `{suffix}` tables (branch-from-box: reuses production loop_cbg/bddp), and run the 6-3a cohort flow + analyses 8-1/2/3/4/5/8 + IR-1 into `outputs/*{suffix}/` (default: the unsuffixed 0.80 primary); exports BOX_TABLES (the box-affected subtree list) for the production_runs/ drivers
│   ├── preset_counts.sql                  — Preset-activation counts behind Table 8.4a (§8-4 cohort): activations + distinct users by dosing mode, cohort denominator + paired-N, per-preset-name breakdown; primary (unsuffixed) + parallel `_box070` / `_box090` sections
│   ├── ir-6_extreme_preset_summary.sql     — IR6 summary (clean successor to the diagnostic counts file): three tables, one per grain — A box-0.80 transition windows / B cohort users' version-eligible activations dataset-wide / C all eligible AB users' qualifying activations (IR-1002 universe + first-AB-day rule) — each stacking the 4 joint grid cells (primary per the 2026-08-20 interpretation), the 4 IR6 marginals (add-on), and any_extreme; raw + 24h-capped exposure (indefinite-override end-of-data clip inflates raw); plus per-grain user-level co-occurrence tabulations (which grid cells / marginal categories each user touched)
│   ├── ir-6_extreme_preset_counts.sql      — IR6 scoping: activations/users/exposure-hours at or beyond the guardrail extremes (needs ≤ 15% / ≥ 200%; own target in the 67–100 / 180–250 mg/dL band, ±0.5 mg/dL mmol-roundtrip tolerance) in the box-0.80 transition cohort; two grains (transition windows via overrides_by_segment; all cohort-user data via overrides_all, version-eligible only), marginal + joint 2×2 combination partition, boundary decomposition, near-bound histograms + exact target pairs, per-user rollup
...
│   └── cohort_diagnosis_breakdown.sql     — Diagnosis-type breakdown (count + %) across all three cohorts (transition/stable/durability) via user_diagnosis_type; the transition view reproduces load_transition_endpoints (both-half CGM-coverage gate); targets the unsuffixed primary transition build
│
├── production_runs/                       — One-click production run drivers (Databricks Run-file entry points)
│   ├── run_all_boxes.py                   — Rebuild + analyze every validity-box build in sequence via run_transition_variant.run(): `""` 0.20/0.80 (report primary, first — rebuilds the prod tables in place) → `_box070` 0.30/0.70 → `_box090` 0.10/0.90; `--only _box070,prod` subset, `--skip-analysis` staging-only
...
### Insulin type in the device data (RFAI, 2026-09)
- Loop 3 records the pod's insulin type (Novolog, Humalog, Apidra, Fiasp, Lyumjev, Afrezza — LoopKit `InsulinType`) at pod change and stamps it on every uploaded dose. Two fields carry it, one per upload route: plugin path `insulinFormulation` → `$.simple.brand` (spelled "NovoLog"); HealthKit path `payload` → `$["com.loopkit.InsulinKit.MetadataKeyInsulinType"]` (spelled "Novolog"). Normalise with `INITCAP(LOWER(...))`. `pumpSettings.insulinModel` (`rapidAdult` / `rapidChild` / `fiasp` / `walsh` / `other`) is Loop's *default rapid-acting curve*, written by the plugin regardless of the pod's insulin — not a brand source.
- A user-day's brand is the brand with the most distinct dose times that day; two-brand days are pod changes. A window's label is its single brand, `multiple`, or `unknown`; nothing is carried in from outside the window (FDA's criticism of the observational study is attribution, so confirmed same-day type is the only source).
- Dosing-day modes follow `export_valid_transition_segments.py`: autobolus = GREATEST(dd, hk autobolus count) ≥ 3; temp_basal = threshold not met and ≥ 1 temp-basal command. Both dosing-day detectors in `loop_recommendations` are Loop-3 signals (dosingDecision uploads; the HealthKit AutomaticallyIssued flag), so Loop 2.x days never enter the day universe and temp-basal days carry a brand about as reliably as autobolus days.
- Limits to disclose: self-report at pod change; no concentration; biosimilars folded into brands; no device-recorded insulin type exists for the 2019–2020 Loop Observational Study period.
