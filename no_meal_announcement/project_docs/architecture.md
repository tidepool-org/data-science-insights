...
**Next:** **implement the §8.3 two-view rank terciles** (same-user-set gated, overall- + CE=0-reference) to replace the now-dropped degenerate empirical terciles before any TDD-stratum claim (D12); high-TDD outlier winsorization before strong High-stratum claims; flesh out the §8.2/§8.3 runnable integration checks (currently stubs); the older `testing/data_staging/` + `testing/analysis/` unit tests are still prior-scaffold stubs. See the latest `project_history.md` entries.
...
│   ├── analysis_8-3_nma_tdd_stratified.py              — §8.3 within-user TDD stratification of CE=0 days (Low/High R=tdd/mean; Tables 8.3a/b/c, 8.3a has 5 sections incl. CE>0+HMA; Figures 8.3a–e incl. 8.3e TIR-vs-TDD-percentile scatter); per-cohort run()/main(); + Appendix §12.3 supplement: median- & rolling-30d-reference variants (per-user table + violin + within-user each) + HMA within-user + LMM (table_12_3a–f, figs 12.3a/12.3c) (D18 + reorg). Empirical terciles dropped. ⚠️ TDD results need scrutiny (D12)
...
Phase 4: Analysis
  ...
  §8.3 Within-user TDD stratification on CE=0 days — implemented in analysis_8-3_nma_tdd_stratified.py.
    ...
    Sensitivities live in the Appendix §12.3 supplement (below). Empirical-tercile outputs were dropped (pending the D12 rank-tercile rework).
