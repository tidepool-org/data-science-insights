...
│   ├── analysis_8-3_nma_tdd_stratified.py              — §8.3 within-user TDD stratification of CE=0 days (Low/High R=tdd/mean; Tables 8.3a/b/c + supp Low/Mid/High terciles; Figures 8.3a–e incl. 8.3e TIR-vs-TDD-percentile scatter); per-cohort run()/main(); + CE>=3/BE>=3 HMA arm stratified Low/High as a 3rd group in figs 8.3a/b/c + Appendix §12.3 table_12_3a_high_engagement_tdd_strata (D18). ⚠️ tercile/TDD results need scrutiny (see Open Questions)
...
Phase 4: Analysis
  ...
  §8.3 Within-user TDD stratification on CE=0 days — implemented in analysis_8-3_nma_tdd_stratified.py.
    ...
    HMA arm (D18): CE>=3/BE>=3 days are stratified Low/High by within-user TDD with the same _ce0_strata machinery and shown as a 3rd group (bronze) in figs 8.3a (6 violins) / 8.3b (3rd Low−High overlay) / 8.3c (4th reference bar); a parallel within-user Low−High contrast is emitted as Appendix §12.3 table_12_3a_high_engagement_tdd_strata.csv (reuses table_8_3b_within_user). ⚠️ inherits the D12 not-citable caveat.
