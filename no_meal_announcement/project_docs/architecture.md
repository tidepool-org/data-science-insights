...
**§8.4 — delivery strategy (AB vs TB) × TDD stratum × day type + carb-entry-rate (2026-06-06, D19).** A new `analysis_8-4_nma_by_delivery_strategy_stratified.py` (SECONDARY/EXPLORATORY) bundles both `delivery_strategy` objectives. **Part 1** crosses AB-vs-TB with the §8.3 same-user-set-gated **within-user TDD rank** strata (overall reference), per day type: a per-day-type 2-way interaction LMM `outcome ~ tdd_stratum * delivery_strategy + (1|user)` (reuses `lmm_day_strategy_interaction` with `day_type_col="tdd_stratum"`) — the **stratum×strategy interaction** (does the AB−TB gap differ Low vs High?) is the inferential headline, the **equal-user-weight cross-tab** the Method-A anchor (D5: no directional claim where they diverge). The main **tables** 8.4a (cross-tab) + 8.4b (interaction) cover **all 5 day types** (3 nested NMA + CE>0 + HMA); the main summary **figure** 8.4a stays the headline **CE=0/BE≤1 vs CE>0** (a 5-day-type figure is too busy — the all-5 figure is Appendix fig 12.4b). Both the **rank-binary** Low/High (the cell-viable main axis, `figure_8_4a` + `table_8_4a`/`table_8_4b`) AND the **rank-tercile** are built ("sketch both"; tercile → §12.4). **Composite same-user-set gate** (users present in every stratum×strategy cell) keeps cells apples-to-apples (verified PASS — equal n_users across a day type's cells); thin High-TDD×TB cells are guarded → `converged=False` (n surfaced). **Part 2** — within-user paired AB-vs-TB carb-logging (`table_8_4c` + `figure_8_4b`): users log carbs on a *smaller* fraction of TB days than AB days (the "surprising direction"). Appendix **§12.4**: tercile cross-tab, CE=0-ref companion, within-stratum within-user AB−TB contrasts (`table_12_4a/c/d`, headline pair) + the all-5-day-type figure (`figure_12_4b`, the figure companion of Table 8.4a) + figs `12_4a/c`. ⚠️ **same-day entanglement** (strategy is endogenous) + D11 → descriptive only; **inherits §8.3's D12 status** (citable; D12 resolved). The §8.3 rank machinery was hoisted **verbatim** to `utils/strata.py` (shared by §8.3 + §8.4; §8.3 outputs unchanged — verified), and a shared `STRATEGY_ALPHA` (AB darker / TB lighter) added to `utils/plotting.py` (§8.2 untouched). See decisions.md **D19**.

**Next:** the §8.3 two-view rank terciles + high-TDD-outlier evaluation are **done (D12 RESOLVED, 2026-06-05)** and the §8.4 delivery-strategy × TDD + carb-entry-rate analysis is **done (D19, 2026-06-06)** — remaining: flesh out the §8.2/§8.3/§8.4 runnable integration checks (currently stubs); the older `testing/data_staging/` + `testing/analysis/` unit tests are still prior-scaffold stubs. See the latest `project_history.md` entries.
...
│   ├── analysis_8-4_nma_by_delivery_strategy_stratified.py  — §8.4 (SECONDARY) delivery strategy (AB/TB) × TDD stratum × day type + carb-entry-rate. Part 1: per-day-type 2-way interaction LMM tdd_stratum × delivery_strategy (Tables 8.4a cross-tab + 8.4b interaction — all 5 day types; Fig 8.4a staggered 95% CI bars, headline CE=0/BE≤1 vs CE>0, rank-binary overall-ref, composite same-user-set gated). Part 2: within-user paired AB−TB carb-logging (Table 8.4c, Fig 8.4b). Appendix §12.4: tercile + CE=0-ref cross-tabs + within-stratum within-user contrast (table_12_4a/c/d, headline pair) + the all-5-day-type figure (figs 12_4a/b/c). Reuses utils/strata; per-cohort run()/main() + --figures-only/--figs (D19)
...
│       ├── data_loader.py                              — shared snapshot loader, §7.6 cohort filter, CE>0 (+ CE>=3/BE>=3) comparator restriction, endpoint/classification constants (ENDPOINTS, CLASSIFICATIONS, MIN_AGE, STRATEGIES/STRATEGY_COL, WINDOW_DAYS/HALF, HIGH_MA_FLAG/SUPPLEMENT_ARMS, …), windowed_matched_means (§8.1 windowed sensitivity, D15), by-path statistics loaders; consumed by §8.1 + §8.2 + §8.3 + §8.4
│       ├── statistics.py                               — cluster_bootstrap_ci, paired_within_user, lmm_arm_contrast (§8.1 Method B), lmm_day_strategy_interaction (§8.2 + reused by §8.4 with day_type_col=tdd_stratum), lmm_tdd_stratum (§8.3); wraps FDA statistics by path
│       ├── strata.py                                   — within-user TDD-stratification toolkit hoisted verbatim from §8.3 (shared by §8.3 + §8.4): _rank_strata (overall/ce0 × binary/tercile) + _same_user_set_gate + _ce0_strata + table_8_3a/b/c base tables + _rank_sections/table_rank_* + _stratum_delta/_tercile_trend_stats + MIN_REF_DAYS/R_CUT/RANK_* constants (D19)
│       └── plotting.py                                 — shared figure conventions for §8.1–§8.4 + supplement: house-style rcParams (larger fonts everywhere) + font-size constants, range-based ENDPOINT_COLORS (Tidepool brand for the 3 non-range metrics), DAY_TYPE_COLORS (fixed per-day-type palette), HIGH_MA_COLOR/HIGH_MA_ALPHA (bronze CE>=3/BE>=3 arm; D18), STRATEGY_ALPHA (AB darker / TB lighter; §8.4 — §8.2 keeps its own local copy; D19), the two 2×2 metric GRIDS (target+safety / hyper+overall), violin_box_panel (dots-behind/box-on-top), overlay_hist_panel (shared bin edges + mean lines)
...
- **TDD / tercile results — D12 RESOLVED (2026-06-05).** §8.3's earlier degeneracy (empirical terciles over **unequal user sets**, + TIR **band-insensitivity** at odds with figure_8_3e) is fixed by the **two-view same-user-set-gated rank terciles** (above; decisions.md D12 RESOLUTION): the gate equalizes n_users across strata, and the overall-reference TIR is cleanly monotonic. Rank-tercile outputs (Table 8.3d + §12.3g–j) are **citable** (D12 resolved); the magnitude strata (8.3a/b/c, 12.3a–f) are superseded by the rank views.
