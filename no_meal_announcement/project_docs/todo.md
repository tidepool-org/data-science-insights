...
- [ ] **§8.3 × delivery-strategy (AB vs TB) — the final big §8.3 step** (plan §8.3 step 4). Break the TDD-stratum analysis by `delivery_strategy` (autobolus_on vs temp_basal_only; `STRATEGIES`/`STRATEGY_COL` in data_loader). Target shape (per MJC): **one summary figure in the main §8.3** + a **messier full breakout in Appendix §12.3**. Design questions to settle in planning: which stratification to cross with strategy (overall-ref rank terciles? the mean-ref Low/High binary?); whether to cross all 5 day types or just the CE=0 arm; how to guard/flag thin cells (esp. High-TDD × TB); relation to §8.2's existing day_type × strategy interaction LMM (reuse `lmm_day_strategy_interaction`?). See the ready-to-use planning prompt in project_history (2026-06-05) / ask MJC.
