...
> rewiring to the current `data_staging/` modules before they will run. Per instructions, the
> testing suite has not been updated.
...
Phase 4: Analysis
  §8.1 NMA-like vs CE>0 contrasts.
    Method A (per-user paired) implemented in analysis_8-1_glycemic_outcomes_nma_vs_carb_entry.py
      -> method_a_contrasts.csv, method_a_panel_a.png (per-user arm means),
         method_a_panel_b_{central,lows,highs}.png (delta histograms stacked by classification),
         figure_8_1a_stacked_bars.png (4-arm time-in-range).
    Method B (LMM) deferred.
  §8.2 Day-type x delivery-strategy interaction — deferred (stub exists from prior scaffold).
  §8.3 Within-user TDD stratification on CE=0 days — deferred (stub exists from prior scaffold).
```
...
| `FDA_real_world_data/analysis/utils/statistics.py` | Loaded by file path in analysis_8-1 for Shapiro / paired-t / Wilcoxon / p-formatting. |
